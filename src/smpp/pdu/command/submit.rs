...
#[cfg(test)]
mod test {
    use crate::smpp::pdu::{command::SubmitSm, prelude::COctet};

    #[test]
    fn test() {
        let s = super::SubmitSm {
            service_type: COctet::new("").unwrap(),
            source_addr_ton: 0,
            source_addr_npi: 0,
            source_addr: COctet::new("447111222222").unwrap(),
            dest_addr_ton: 0,
            dest_addr_npi: 0,
            destination_addr: COctet::new("447000123123").unwrap(),
            esm_class: 0,
            protocol_id: 1,
            priority_flag: 1,
            schedule_delivery_time: COctet::new("").unwrap(),
            validity_period: COctet::new("").unwrap(),
            registered_delivery: 1,
            replace_if_present_flag: 0,
            data_coding: 1,
            sm_default_msg_id: 0,
            sm_length: 4,
            short_message: "hihi".to_owned().into_bytes(),
            msg_submission_tlv: vec![],
        };
        ...
        let dec = bincode::decode_from_slice::<SubmitSm, _>(
            &enc,
            bincode::config::standard().with_big_endian().with_fixed_int_encoding(),
        )
        .unwrap();
